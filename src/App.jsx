...
const SEED_QUESTIONS = [ /* ... same questions as before ... */ ];
...
function makeBlobs(n){ return Array.from({length:n},(_,i)=>({ id:`b${i}`, x:Math.random()*70+10, y:Math.random()*70+10, r:Math.random()*10+8, alpha:0.85 })); }
...
export default function FluitecVarnishCleaner(){
  ...
  const [blobs,setBlobs]=useState(()=>makeBlobs(100)); // fully covered at start with 100 blobs
  ...
  function answer(idx){
    ...
    if(correct){
      setScore(s=>s+1);
      setBlobs(bs=>bs.slice(0, bs.length - Math.floor(100/10))); // remove 10% each correct
    }
    setTimeout(()=>setRound(r=>r+1),2000);
  }

  function restart(){ setSeries(pickTenProgressive(bank)); setBlobs(makeBlobs(100)); setRound(0); setScore(0); setFeedback(null); setSelectedIdx(null); }
